...
#if defined(TEST_DS_TWR_RESPONDER_CP)
...
#define POLL_RX_TO_RESP_TX_DLY_UUS 5388
...
#define FINAL_RX_TIMEOUT_UUS 4483
...
static uint16_t fp_int_T2;
static uint16_t fp_int_T6;
static uint64_t T2_timestamp;
static uint64_t T6_timestamp;
...
int ds_twr_responder_cp(void)
{
    ...
    while (1)
    {
        memset(&rx_diag1,0,sizeof(rx_diag1));
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                {
                    ...
                    if (frame_len <= RX_BUF_LEN)
                    {
                        dwt_readrxdata(rx_buffer, frame_len, 0); // anchor received final ========
                    }

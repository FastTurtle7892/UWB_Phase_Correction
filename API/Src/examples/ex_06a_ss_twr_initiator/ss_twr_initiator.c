...
uint32_t POLL_TX_TO_RESP_RX_DLY_UUS = 240;
uint32_t RESP_RX_TIMEOUT_UUS = 400;
...
int ss_twr_initiator(void)
{
    ...
    getDelayTime_ss_twr_initiator(&config, &POLL_TX_TO_RESP_RX_DLY_UUS, &RESP_RX_TIMEOUT_UUS);
    ...
    while (1)
    {
        ...
        else
        {
          ...
          {
            timer_init_uss();
            printf("CNT = %d", pkt_cnt);
            printf("\t T : ");

            for(int j=0; j<pkt_cnt; j++)
              printf("%f \t", distance_storage[j]);

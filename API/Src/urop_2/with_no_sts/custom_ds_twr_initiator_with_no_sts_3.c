...
#if defined(CUSTOM_DS_TWR_INITIATOR_WITH_NO_STS_3)
...
static uint8_t tx_post_final_msg[] = {0x41, 0x88, 0, 0xCA, 0xDE, 'W', 'A', 'V', 'E', 0x12, 0x02, 0, 0, 0, 0, 0, 0, 0, 0};
...
#define POLL_TX_TO_RESP_RX_DLY_UUS 1550
...
#define FINAL_TX_POST_FINAL_TX_DLY_UUS 1700
...
int custom_ds_twr_initiator_with_no_sts_3(void)
{
    ...
    while (1)
    {
        ...
        if (ret == DWT_SUCCESS)
        {
          ...
          uint32_t res_rx_time = (POLL_TX_TO_RESP_RX_DLY_UUS * UUS_TO_DWT_TIME) >> 8; 
          ...
          if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
          {
              ...
              if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
              {
                  ...
                  final_tx_time = (RESP_RX_TO_FINAL_TX_DLY_UUS * UUS_TO_DWT_TIME) >> 8;
                  ...
                  ret = dwt_starttx(DWT_START_TX_DLY_RS);
                  ...
                  if (ret == DWT_SUCCESS)
                  {
                      ...
                      uint32_t post_final_tx_time = (FINAL_TX_POST_FINAL_TX_DLY_UUS * UUS_TO_DWT_TIME) >> 8;
                      dwt_setdelayedtrxtime(post_final_tx_time);

                      tx_post_final_msg[ALL_MSG_SN_IDX] = frame_seq_nb;
                      snprintf(&tx_post_final_msg[11], sizeof(tx_post_final_msg) - 11, "%.6f", response_cir_phase);
                      ...
                      ret = dwt_starttx(DWT_START_TX_DLY_TS);
                      ...
                      if (ret == DWT_SUCCESS)
                      {
                        ...
                        uint32_t poll_tx_time = (1700 * UUS_TO_DWT_TIME) >> 8;

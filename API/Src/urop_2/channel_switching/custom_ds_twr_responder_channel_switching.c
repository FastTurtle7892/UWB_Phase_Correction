...
#if defined(CUSTOM_DS_TWR_RESPONDER_CHANNEL_SWITCHING)
...
double f_CH5 = 6.4896e9;
double f_CH9 = 7.9872e9;
...
static uint8_t rx_post_final_msg[] = {0x41, 0x88, 0, 0xCA, 0xDE, 'W', 'A', 'V', 'E', 0x12, 0x02, 0, 0, 0, 0, 0, 0, 0, 0};
...
#define POLL_RX_TO_RESP_TX_DLY_UUS 1370
...
#define RESP_TX_TO_FINAL_RX_DLY_UUS 1170
/* Receive final timeout. See NOTE 5 below. */
#define FINAL_RX_TIMEOUT_UUS 290

#define FINAL_RX_POST_FINAL_RX_DLY_UUS 1220


#define POST_FINAL_TIMEOUT_UUS 288
...
static double phase_ch9_dist_change = 0;
...
KalmanFilter kf_N;
KalmanFilter kf_phase;

char uart_send_data[10];
#define UART_HWFC APP_UART_FLOW_CONTROL_ENABLED

int custom_ds_twr_responder_channel_switching(void)
{
    ...
    FILE *test;
    test = fopen("test_CH5_09_27_2.txt", "w");


    int cnt = 0;

    //kalman_init(&kf_N, 0.1, 3, 0.0);
    //kalman_init(&kf_phase, 0.1, 3, 0.0);
    init_moving_average(15);
    ...
    while (1)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
                ...
                if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                {
                    ...
                    if (memcmp(rx_buffer, rx_final_msg, ALL_MSG_COMMON_LEN) == 0)
                    {
                        ...
                        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                        {
                            ...
                            {    
                              //printf("\npost final recv ok");
                              snprintf(temp, sizeof(temp), "%s", &rx_buffer[11]);
                             ...
                             /* Compute time of flight. 32-bit subtractions give correct answers even if clock has wrapped. See NOTE 12 below. */
                              ...
                              //printf("\nPoll_cir : %f", cir_phase[0]);
                              //printf("\nResponse_cir : %f", cir_phase[1]);
                              //printf("\nFinal_cir : %f", cir_phase[2]);
                              //printf("\nPost_Final_cir : %f", cir_phase[3]);
                                ...
                                reset_DWIC(); 
                                 ...
                                 {
                                      ch9_cnt++;
                                      distance_ch9 = distance;
                                      phase_ch9 = normalize_phase(normalize_phase(cir_phase[0] + cir_phase[1]) - normalize_phase(cir_phase[2] - cir_phase[3]));
                                      filtered_phase_ch9 = apply_moving_average((phase_ch9));
                                      printf("\nfiltered_phase_ch9 : %f", filtered_phase_ch9);
                                      //printf("\nphase_ch9 : %f\n", phase_ch9);
                                 ...
                                 {
                                      ch5_cnt++;
                                      distance_ch5 = distance;
                                      phase_ch5 = normalize_phase((cir_phase[0] + cir_phase[1]) - (cir_phase[2] - cir_phase[3]));
                                      filtered_phase_ch5 = apply_moving_average((phase_ch5));
                                      //printf("\nfiltered_phase_ch5 : %f", filtered_phase_ch5);
                                      //printf("\phase_ch5 : %f\n", phase_ch5);
                                      ...
                                      dwt_setrxantennadelay(CH9_ANT_DLY);
                                      dwt_settxantennadelay(CH9_ANT_DLY);
                                      state = 1;
                                 }
                            }
         ...
         {
            phase_ch5_dist_change = filtered_phase_ch5 * (c_speed / (2*f_CH5)) / (2*PI);
            phase_ch9_dist_change = filtered_phase_ch9 * (c_speed / (2*f_CH9)) / (2*PI);
            phase_ch5_ch9_dist_change = (filtered_phase_ch9 - filtered_phase_ch5) * new_wavelength / (2*PI);

            int dist_ch5_N = (int)(distance_ch5 / (c_speed / (2*f_CH5)));
            int dist_ch9_N = (int)(distance_ch9 / (c_speed / (2*f_CH9)));

            double new_dist_ch9 = (double)dist_ch9_N * (c_speed / (2*f_CH9)) + phase_ch9_dist_change;


            new_dist_N = (int)(distance_ch9 / new_wavelength);
            
            new_dist = (double)new_dist_N * new_wavelength + (phase_ch5_ch9_dist_change);
            

            final_dist_N = (int)(new_dist / (c_speed / (2*f_CH9)));
            final_dist = (double)final_dist_N * (c_speed / (2*f_CH9)) + phase_ch9_dist_change;

...
#if defined(CUSTOM_DS_TWR_RESPONDER_CHANNEL_SWITCHING_V2)
...
#define RESP_TX_TO_FINAL_RX_DLY_UUS 1800
/* Receive final timeout. See NOTE 5 below. */
#define FINAL_RX_TIMEOUT_UUS 291
...
static double measured_dist_CH5 = 0;
static double measured_dist_CH9 = 0;
static double rec_phase_CH5 = 0;
static double rec_phase_CH9 = 0;
static double delta_rec_phase = 0;
static double new_dist = 0;
static double final_dist = 0;
int final_dist_N = 0;
int new_dist_N = 0;
...
int custom_ds_twr_responder_channel_switching_v2(void)
{
    ...
    while (1)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            volatile int fp_int_shift = fp_int;
            
            dwt_readaccdata(accum_data, ACCUM_DATA_LEN, fp_int_shift);
            cir_phase[0] = calculate_CIR(accum_data, ACCUM_DATA_LEN);
            
            
            //fprintf(test, "|POLL FIRST_PATH CIR END\n");
            ...
            if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                {
                    ...
                    fp_print = rx_diag.ipatovFpIndex/64.;
                        ...
                        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                        {
                            ...
                            {    
                             ...
                             /* Compute time of flight. 32-bit subtractions give correct answers even if clock has wrapped. See NOTE 12 below. */
                              ...
                              measured_dist_CH5 = distance;

                              rec_phase_CH5 = (cir_phase[0] + cir_phase[1] - cir_phase[2] + cir_phase[3]) * (c_speed / (2*f_CH5))/ (2*PI);
                              ch5_cnt++;
                              ...
                              reset_DWIC();  // change ch9

                              if(dwt_configure(&config_CH9)) 
                              {
                                  test_run_info((unsigned char *)"CONFIG FAILED     ");
                                  while (1)
                                  { };
                                              ...
                                              if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                                              {
                                                  ...
                                                  {    
                                                   ...
                                                   /* Compute time of flight. 32-bit subtractions give correct answers even if clock has wrapped. See NOTE 12 below. */
                                                    ...
                                                    measured_dist_CH9 = distance;

                                                    rec_phase_CH9 = (cir_phase[0] + cir_phase[1] - cir_phase[2] + cir_phase[3]) * (c_speed / (2*f_CH5)) / (2*PI);
                                                    ch9_cnt++;
                                                    
                                                    delta_rec_phase = rec_phase_CH9 - rec_phase_CH5;

                                                    new_dist_N = measured_dist_CH9 / new_wavelength;

                                                    new_dist = new_dist_N * new_wavelength + delta_rec_phase;

                                                    for(int i=0; i<4; i++)
                                                      cir_phase[i] = 0;

                                                    ///////////////// single frequency (ch9) change //////////////////

                                                    final_dist_N = new_dist / (c_speed / (2*f_CH9));
                                                    
                                                    final_dist = final_dist_N * (c_speed / (2*f_CH9)) + rec_phase_CH9;
                                                    ...
                                                    reset_DWIC();  // change ch5

                                                    if(dwt_configure(&config_CH5)) 
                                                    {
                                                        test_run_info((unsigned char *)"CONFIG FAILED     ");
                                                        while (1)
                                                        { };

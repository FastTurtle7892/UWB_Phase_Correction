...
#if defined(CUSTOM_DS_TWR_RESPONDER_SINGLE_FREQUENCY_ANCHOR_2_1VS2)
...
int custom_ds_twr_responder_single_frequency_anchor_2_1vs2(void)
{
    ...
    MedianFilter filter_n; // N value
    init_filter(&filter_n);

    MedianFilter filter_p; // phase value
    init_filter(&filter_p);
                          ...
                          if (memcmp(rx_buffer, rx_final_msg, ALL_MSG_COMMON_LEN) == 0)
                          {
                              ...
                              if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                              {
                                  ...
                                  {    
                                    ...
                                    poll_rx_time = (post_final_rx_ts + (8100 * UUS_TO_DWT_TIME)) >> 8;
                                   ...
                                   /* Compute time of flight. 32-bit subtractions give correct answers even if clock has wrapped. See NOTE 12 below. */
                                    ...
                                    filtered_rec_phase_dist = process_value(&filter_p, raw_rec_phase_dist_change);

                                    // N
                                    raw_N = (int)(distance / CH9_wavelength); 
                                    filtered_N = process_value(&filter_n, raw_N);

                                    // recovery dist
                                    new_distance = (double)filtered_N * CH9_wavelength + filtered_rec_phase_dist;

                                    //printf("\ndistance : %3.4f m\n", distance);
                                    printf("\n%d|%.4f", cnt, new_distance);
                              
                                    cnt++;
                              
                                    //sprintf(uart_send_data, "%d|%3.4f\n", cnt, new_distance);

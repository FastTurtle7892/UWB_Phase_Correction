...
#if defined(CUSTOM_DS_TWR_RESPONDER_MULTI_FREQUENCY_ANCHOR1)
...
static dwt_config_t config_CH5 = {
...
static dwt_config_t config_CH9 = {
...
#define CH5_ANT_DLY 16345
#define CH9_ANT_DLY 16345


double new_wavelength = 299792458.0 / (2 * (7.9872e9 - 6.4896e9));
...
#define RESP_TX_TO_FINAL_RX_DLY_UUS 1850
/* Receive final timeout. See NOTE 5 below. */
#define FINAL_RX_TIMEOUT_UUS 298

#define FINAL_RX_POST_FINAL_RX_DLY_UUS 1850
...
static double distance_ch5 = 0;
static double distance_ch9 = 0;
static double phase_ch5 = 0;
static double phase_ch9 = 0;
static double kalman_phase_ch9 = 0;
static double phase_ch5_dist_change = 0;
double phase_ch9_dist_change = 0;
static double phase_ch5_ch9_dist_change = 0;
static double filtered_phase_ch5_ch9_dist_change = 0;
static double filtered_phase_ch9 = 0;
static double filtered_phase_ch5 = 0;
static double phase_ch9_filtered_dist_change = 0;
int filtered_N_ch9 = 0;
int filtered_N_ch5 = 0;
static double filtered_phase_ch9_dist_change = 0;
static double filtered_phase_ch5_dist_change = 0;
int raw_N_ch9 = 0;
int raw_N_ch5 = 0;
static double new_distance_ch9 = 0;
static double new_distance_ch5 = 0;


int new_dist_N = 0;
int filtered_new_dist_N = 0;
double new_dist = 0;

int final_dist_N = 0;
int filtered_final_dist_N = 0;
double final_dist = 0;
...
int custom_ds_twr_responder_multi_frequency_anchor1(void)
{
    ...
    if(dwt_configure(&config_CH5)) /* if the dwt_configure returns DWT_ERROR either the PLL or RX calibration has failed the host should reset the device */
    {
    ...
    dwt_setrxantennadelay(CH5_ANT_DLY);
    dwt_settxantennadelay(CH5_ANT_DLY);
    state = 0;
    ch5_cnt++;
    ...
    MedianFilter_N filter_n_ch5; // N value
    init_filter_N(&filter_n_ch5);

    MedianFilter_N filter_n_ch9; // N value
    init_filter_N(&filter_n_ch9);

    MedianFilter_N filter_n_new; // N value
    init_filter_N(&filter_n_new);

    MedianFilter_N filter_n_final; // N value
    init_filter_N(&filter_n_final);

    MedianFilter_PHASE filter_p_ch5; // phase value
    init_filter_PHASE(&filter_p_ch5);

    MedianFilter_PHASE filter_p_ch9; // phase value
    init_filter_PHASE(&filter_p_ch9);

    MedianFilter_PHASE filter_p_new; // phase value
    init_filter_PHASE(&filter_p_new);
    ...
    while (1)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
                        ...
                        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                        {
                            ...
                            nrf_delay_ms(1);
                             ...
                             /* Compute time of flight. 32-bit subtractions give correct answers even if clock has wrapped. See NOTE 12 below. */
                              ...
                              {
                                  ch9_cnt++;
                                  distance_ch9 = distance; // measured distance
                                  phase_ch9 = normalize_phase(cir_phase[0] + cir_phase[1] - cir_phase[2] + cir_phase[3]); // recovery phase
                                  phase_ch9_dist_change = phase_ch9 * CH9_wavelength / (2*PI); // phase to distance
                                  filtered_phase_ch9_dist_change = process_value_PHASE(&filter_p_ch9, phase_ch9_dist_change); // get distance using median filter

                                  // N
                                  raw_N_ch9 = (int)(distance_ch9 / CH9_wavelength); 
                                  filtered_N_ch9 = process_value_N(&filter_n_ch9, raw_N_ch9);

                                  // recovery dist
                                  new_distance_ch9 = (double)filtered_N_ch9 * CH9_wavelength + filtered_phase_ch9_dist_change;

                                  //printf("\nnew distance ch9 : %f", new_distance_ch9);
                               ...
                               {
                                  ch5_cnt++;
                                  distance_ch5 = distance; // measured distance
                                  phase_ch5 = normalize_phase(cir_phase[0] + cir_phase[1] - cir_phase[2] + cir_phase[3]); // recovery phase
                                  phase_ch5_dist_change = phase_ch5 * CH5_wavelength / (2*PI); // phase to distance
                                  filtered_phase_ch5_dist_change = process_value_PHASE(&filter_p_ch5, phase_ch5_dist_change); // get distance using median filter

                                  // N
                                  raw_N_ch5 = (int)(distance_ch5 / CH5_wavelength); 
                                  filtered_N_ch5 = process_value_N(&filter_n_ch5, raw_N_ch5);

                                  // recovery dist
                                  new_distance_ch5 = (double)filtered_N_ch5 * CH5_wavelength + filtered_phase_ch5_dist_change;

                                  //printf("\nnew distance ch5 : %f", new_distance_ch5);

                                  if(dwt_configure(&config_CH9)) /* if the dwt_configure returns DWT_ERROR either the PLL or RX calibration has failed the host should reset the device */
                                  {
                                      test_run_info((unsigned char *)"CONFIG FAILED     ");
                                      while (1)
                                      { };
         ...
         if(distance_ch5 != 0 && distance_ch9 != 0 && !state)
         {
            phase_ch5_ch9_dist_change = (filtered_phase_ch9_dist_change - filtered_phase_ch5_dist_change);
            filtered_phase_ch5_ch9_dist_change = process_value_PHASE(&filter_p_new, phase_ch5_ch9_dist_change);

            new_dist_N = (int)(new_distance_ch9 / new_wavelength);
            filtered_new_dist_N = process_value_N(&filter_n_new, new_dist_N);

            new_dist = (double)filtered_new_dist_N * new_wavelength + (filtered_phase_ch5_ch9_dist_change);
            
            final_dist_N = (int)(new_dist / CH9_wavelength);
            filtered_final_dist_N = process_value_N(&filter_n_final, final_dist_N);

            final_dist = (double)filtered_final_dist_N * CH9_wavelength + filtered_phase_ch9_dist_change;

            //printf("\n%f|%f", new_distance_ch5, new_distance_ch9 + 0.07);
            //printf("\n%f", final_dist);
            printf("\n%f", new_distance_ch9);
            ...
            //printf("\ndist_ch9_N : %d", dist_ch9_N);
            //printf("\ndist_ch5_N : %d", dist_ch5_N);
            //printf("\nnew_dist_N : %d\n", new_dist_N);
            //printf("\nnew_dist : %.4f m\n\n", new_dist);

            //printf("\nphase_ch5 : %f", normalize_phase(phase_ch5));
            //printf("\nphase_ch9 : %f", normalize_phase(phase_ch9));
            //printf("\nphase_ch9 - phase_ch5 : %f\n", normalize_phase(filtered_phase_ch9 - filtered_phase_ch5));
            //printf("\nfinal_dist : %.4f m\n\n", final_dist);


            sprintf(uart_send_data, "%3.4f\n", final_dist);
            uart0_strsend(uart_send_data);
                                 

            distance_ch5 = 0; distance_ch9 = 0;
            phase_ch5 = 0; phase_ch9 = 0;
            new_dist_N = 0; new_dist = 0;
            final_dist_N = 0; final_dist = 0;

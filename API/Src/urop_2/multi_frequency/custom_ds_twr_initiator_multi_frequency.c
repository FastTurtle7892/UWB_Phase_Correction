...
#if defined(CUSTOM_DS_TWR_INITIATOR_MULTI_FREQUENCY)
...
static bool check_ch5 = true;
static bool check_ch9 = false;
...
uint8_t anchor_1_address[] = {'W', 'A'};
uint8_t anchor_2_address[] = {'C', 'H'};
...
#define POLL_TX_TO_RESP_RX_DLY_UUS 1850
...
#define FINAL_TX_POST_FINAL_TX_DLY_UUS 2000
...
int custom_ds_twr_initiator_multi_frequency(void)
{
                ...
                if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
                {
                    ...
                    if (ret == DWT_SUCCESS)
                    {
                        ...
                        if (ret == DWT_SUCCESS)
                        {
                          ...
                          if(check_ch5 && check_ch9)
                          {
                              if(check_anchor_num == 1)
                              {
                                //printf("anchor 1 ok\n");
                                // change address -> anchor 2

...
#if defined(CUSTOM_DS_TWR_INITIATOR_CIR)
...
#define RESP_RX_TO_FINAL_TX_DLY_UUS 2009
 ...
 * @return none
 */
int custom_ds_twr_initiator_cir(void)
{
    ...
    while (1)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            save_CIR(accum_data, ACCUM_DATA_LEN, response_cir_data, sizeof(response_cir_data)/sizeof(response_cir_data[0]));
            ...
            if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                if (ret == DWT_SUCCESS)
                {
                    ...
                    memcpy(&tx_post_final_msg[11], &response_cir_data[0], sizeof(int32_t));
                    memcpy(&tx_post_final_msg[15], &response_cir_data[1], sizeof(int32_t));
                    snprintf(&tx_post_final_msg[19], sizeof(tx_post_final_msg) - 19, "%.6f", response_cir_phase);

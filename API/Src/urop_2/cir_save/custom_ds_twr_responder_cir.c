...
#if defined(CUSTOM_DS_TWR_RESPONDER_CIR)
...
double cir_phase[4] = {0, 0, 0, 0};

int32_t poll_cir_data[2] = {0,0};
int32_t response_cir_data[2] = {0,0};
int32_t final_cir_data[2] = {0,0};
int32_t post_final_cir_data[2] = {0,0};
...
int custom_ds_twr_responder_cir(void)
{
    ...
    while (cnt<500)
    {
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            save_CIR(accum_data, ACCUM_DATA_LEN, poll_cir_data, sizeof(poll_cir_data)/sizeof(poll_cir_data[0]));
            ...
            if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                {
                    ...
                    save_CIR(accum_data, ACCUM_DATA_LEN, final_cir_data, sizeof(final_cir_data)/sizeof(final_cir_data[0]));
                    ...
                    if (memcmp(rx_buffer, rx_final_msg, ALL_MSG_COMMON_LEN) == 0)
                    {
                        ...
                        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
                        {
                            ...
                            printf("\n%f", cir_phase[3]);
                            save_CIR(accum_data, ACCUM_DATA_LEN, post_final_cir_data, sizeof(post_final_cir_data)/sizeof(post_final_cir_data[0]));
                            ...
                            {    
                              snprintf(temp, sizeof(temp), "%s", &rx_buffer[19]);
                              cir_phase[1] = strtod(temp, NULL);

                              memcpy(&temp_real, &rx_buffer[11], 4);
                              memcpy(&temp_img, &rx_buffer[15], 4);
                              response_cir_data[0] = temp_real;
                              response_cir_data[1] = temp_img;
                             ...
                             /* Compute time of flight. 32-bit subtractions give correct answers even if clock has wrapped. See NOTE 12 below. */
                              ...
                              //printf("\nPost_Final_cir : %f", cir_phase[3]);
                                ...
                                printf("\ndistance : %f m\n\n", distance);
                                fprintf(test, "%d | %d | %d | %d | %d | %d | %d | %d | %d | ", cnt, poll_cir_data[0], poll_cir_data[1], response_cir_data[0], response_cir_data[1], final_cir_data[0], final_cir_data[1], post_final_cir_data[0], post_final_cir_data[1]);
                                fprintf(test, "%f | %f | %f | %f\n", cir_phase[0], cir_phase[1], cir_phase[2], cir_phase[3]);
          ...
          poll_cir_data[0] = 0;
          poll_cir_data[1] = 0; 
          response_cir_data[0] = 0;
          response_cir_data[1] = 0; 
          final_cir_data[0] = 0;
          final_cir_data[1] = 0; 
          post_final_cir_data[0] = 0;
          post_final_cir_data[1] = 0; 
